...
app.get('/api/v1/unique-users', asyncWrapper(async (req, res) => {
  const uniqueUsers = await UserModel.aggregate([
  ...
  const data = {
    labels: uniqueUsers.map(({ month, year }) => `${year}-${month.toString().padStart(2, '0')}`),
    datasets: [
      {
        label: "Unique API Users",
        data: uniqueUsers.map(({ count }) => count),
